	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE 'CN_ENTRY_TABLE'" ) != CN_ENTRY_TABLE ) {
		...
		if ( version_compare( $dbVersion, '0.1.2', '<' ) ) {
			...
			foreach ( $results as $result ) {
				...
				if ( is_array( $options ) && isset( $options['entry']['type'] ) ) {
					$entryType = sanitize_key( $options['entry']['type'] );
				} else {
					$entryType = 'individual';
				}

				$wpdb->query( $wpdb->prepare( 'UPDATE ' . CN_ENTRY_TABLE . ' SET entry_type = %s WHERE id = %d', $entryType, (int) $result->id ) );
			}
		...
		if ( version_compare( $dbVersion, '0.1.6', '<' ) ) {
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->addresses ) ) {
					$addresses = maybe_unserialize( $result->addresses );
					$order     = 0;
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->phone_numbers ) ) {
					$phoneNumbers = maybe_unserialize( $result->phone_numbers );
					$order        = 0;
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->email ) ) {
					$emailAddresses = maybe_unserialize( $result->email );
					$order          = 0;
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->im ) ) {
					$imIDs = maybe_unserialize( $result->im );
					$order = 0;
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->social ) ) {
					$socialMediaIDs = maybe_unserialize( $result->social );
					$order          = 0;
			...
			foreach ( $results as $result ) {
				if ( ! empty( $result->websites ) ) {
					$websites = maybe_unserialize( $result->websites );
					$order    = 0;
			...
			foreach ( $results as $result ) {
				...
				$wpdb->query( $wpdb->prepare( 'UPDATE ' . CN_ENTRY_TABLE . ' SET slug = %s WHERE id = %d', $slug, (int) $result->id ) );
			}
